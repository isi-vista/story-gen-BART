...
def title_key(title_file, kw_file, title_key_file):
    ...
    with open(title_file, mode="r") as infile:
        titles = json.load(infile)
    with open(kw_file, mode="r") as infile:
        keywords = [line.strip() for line in infile]
    print(f"Concat {len(titles)} titles with {len(keywords)} storylines")
    with open(title_key_file, mode="w") as outfile:
        for title, kw in zip(titles, keywords):
            outfile.write(title_kw_pattern.format(title, kw))
            outfile.write("\n")
...
def title_key_story(title_file, kw_file, story_file, title_key_story_file):
    ...
    print("Concat {0} title with {1} storylines and {2} stories".format(len(titles), len(keywords),
                                                                        len(stories)))
...
if __name__ == "__main__":
    ...
    parser.add_argument('--title_key_story', action='store_true',
                        help='concat title, storyline and story')
